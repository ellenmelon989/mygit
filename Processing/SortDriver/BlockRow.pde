class BlockRow {
  ...
  void insertionSortOnce() {
    ...
    if (sortedEnd != row.length) {
      compare++;
      //store the new value we are inserting
      //in a new variable
      Block blockToInsert = new Block(row[newValue].cx, row[newValue].cy, row[newValue].sideLength);
      ...
      if (currentPos >= 0 && row[currentPos].sideLength > blockToInsert.sideLength) {
        ...
        row[newValue].sideLength = row[currentPos].sideLength;
